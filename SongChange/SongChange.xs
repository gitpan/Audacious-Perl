...
typedef gint Audacious__Remote;
typedef sc * Audacious__SongChange;
...
static void sc_perl_change(sc *obj, gint pos) {
	...
	sv_setiv(newSVrv(sv_session, "Audacious::Remote"), 
		 obj->session);
	sv_setiv(sv_pos, pos);
...
static void sc_DESTROY(sc *obj)
{
     ...
     * but we only ever have on Audacious::SongChange object alive
     * so nothing "leaks"
     */
    sc_stop(obj);
...
MODULE = Audacious::SongChange   PACKAGE = Audacious::SongChange   PREFIX = sc_
...
sc_new(sv_class, session=0)
    SV *sv_class
    Audacious::Remote session

void
sc_DESTROY(obj)
    Audacious::SongChange obj
