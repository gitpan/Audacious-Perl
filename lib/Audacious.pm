...
sub helpstr {
    ...
    my $cmds = Audacious->Cmds;
    my @retval;

    for my $cmd (@$cmds) {
	my $cv = Audacious::Cmd->can($cmd);
	next unless defined $cv;
	my $sub = \&{"Audacious::\u${cmd}Cmds"};
	my($options, $args);
	if (defined &$sub) {
	    $options = '[' . join('|', @{ $sub->() }) . ']';
	}
...
my %jtime;
{
    package Audacious::Jtime;
    no strict;
    @ISA = qw(Audacious::SongChange);
...
eval {
    require Audacious::SongChange;
};
if ($@) {
    undef &Audacious::Cmd::crop;
}

sub init_songchange {
    return if $sc;
    $sc = Audacious::SongChange->new($remote);
    tie %jtime, 'Audacious::Jtime', $sc;
}
...
sub init_urldb {
    if (open FH, "$ENV{HOME}/.audacious/.perlurldb") {
	while (<FH>) {
	    s/^\s+//;
	    next unless /^http:/;
	    s/\s+$//;
...
sub init {
    $config = Audacious::Config->new(Audacious::Config->file);
    $pconfig = Audacious::Config->new(Audacious::Config->perlfile);
    $remote = Audacious::Remote->new;

    unless ($remote->is_running) {
	exec "audacious" unless $Pid = fork;
	Audacious::sleep(0.25);
	local $SIG{ALRM} = sub {
	    die "connect to audacious failed: $!";
	};
	alarm 5;
...
sub shell {
    ...
    my $rcfile = "$ENV{HOME}/.audacious/.perlrc";
    if (-e $rcfile) {
	Audacious::Cmd->history("<$rcfile");
    }   
...
sub version {
    print "audacious shell -- remote control v$Audacious::VERSION\n";

    printf "ReadLine support..........%s\n", 
    $term->ReadLine ne "Term::ReadLine::Stub" ? "enabled" :
      "available (install Bundle::Audacious)";
...
sub boot {
    ...
    my $op = $config->read(audacious => 'output_plugin');
    if ($op =~ /disk_writer/) {
	print <<EOF;
*************************************************************
* WARNING:
* Your audacious Output Plugin is set to disk_writer,
...
sub readline::F_Audacious_interrupt {
    readline::F_UnixLineDiscard();
    Audacious::Cmd->quit if $Signal++;
}

sub readline::F_Audacious_volume_up {
    Audacious::Cmd::volume(undef, '+');
}

sub readline::F_Audacious_volume_down {
    Audacious::Cmd::volume(undef, '-');
}

#remote time slider doesnt work so well
#sub readline::F_Audacious_time_up {
#    Audacious::Cmd::time(undef, '+');
#}

#sub readline::F_Audacious_time_down {
#    Audacious::Cmd::time(undef, '-');
#}
...
sub init_keybindings {
	...
	push @code, <<EOF;
    *F_Audacious_$v = sub { Audacious::Cmd::$v(undef) } unless defined &F_Audacious_$v;
    rl_bind('$k', 'xmms_$v');
EOF
    }
    while (my($k,$v) = each %window_bindings) {
	push @code, <<EOF;
    *F_Audacious_window_$v = 
         sub { Audacious::Cmd::window('','$v','') } unless defined &F_Audacious_window_$v;
...
EOF
    ...
    for (1..9) {
	push @code, <<EOF;
	sub F_Audacious_$_ { Audacious::Cmd->track($_) }
	rl_bind('M-$_', 'xmms_$_');
EOF
    }
...
sub run_cmd {
    if ($_[0] =~ s/^\s*\+//) {
	eval "{package Audacious; no strict; @_}";
	Audacious::Cmds(1); #sync
	$Help = ""; #invalidate cache
    }
    else {
	for my $cmd (split /;/, $_[0]) {
	    eval {
		rrun_cmd(Audacious::interp($cmd));
	    };
	}
    }
    print Audacious::highlight(Error => $@) if $@;
}
...
sub resolve {
    my $cmd = shift;
    return $cmd if Audacious::Cmd->can($cmd);
...
sub rrun_cmd {
    ...
    if (my $command = Audacious::resolve($cmd)) {
	Audacious::Cmd->$command($args);
	push @history, $line;
	...
	local $/;
	print {Audacious::pager()} <CMD>;
	close CMD;
    }
    else {
	print Audacious::highlight(Error => "unknown command: `$cmd'\n");
    }
}
...
sub cpl {
    ...
    my $command = defined $rest ? Audacious::resolve($cmd) : $cmd;
    if (Audacious::Cmd->can($command) and prototype("Audacious::Cmd::$command")) {
	$is_cpl = 1;
	my @retval = Audacious::Cmd->$command($rest);
	$is_cpl = 0;
	return @retval;
    }
    ...
    return Audacious::filecomplete((split /\s+/, $rest)[-1]);
}

sub playlist_is_empty {
    my $no_msg = shift;
    unless ($remote->get_playlist_length) {
	print Audacious::highlight(Error => "playlist is empty\n") unless $no_msg;
	return 1;
    }
...
sub filesel_path {
    ...
    $pconfig->read(perl => 'play_path') ||	
      $config->read(audacious => 'filesel_path') ||
	$ENV{MP3_HOME};
}
...
sub defaultpaths {
    '.', filesel_path(), historysel(), "$ENV{HOME}/.audacious";
}
...
sub waitfor (&) {
	...
	alarm 2;
	Audacious::sleep(0.05) while !$true->();
	alarm 0;
    };
}
...
sub open_careful {
	...
	if ($ans =~ /^n/i) {
	    print Audacious::highlight(Warn => "save aborted\n");
	    return;
	}
    }
...
package Audacious::Sort;

sub order {
    my $args = shift;
    my $files = $remote->get_playlist_files;
    my $range = Audacious::range($args);
    my @new;
    my $i = 0;

    for (@$range) {
	$files->[$_-1] =~ /(\d+)\.cda$/;
	$Audacious::CD::order{$i++} = int($1)-1;
	push @new, $files->[$_-1];
    }
...
sub file {
    my(@list) = sort { 
	Audacious::basename($a) cmp Audacious::basename($b);
...

{
	...
	no strict 'refs';
	*$meth = sub { Audacious::File::sort($meth) };
    }
}

sub Audacious::FileInfo::sort {
    my $sort = uc shift;
    my @list;

    for my $file (@{ $remote->get_playlist_files }) {
	my $tag = Audacious::get_mp3tag($file);
	$tag->{FILE} = $file;
	push @list, $tag;
    }
...
sub alias ($$) {
    ...
    *{"Audacious::Cmd::$sub"} = sub (*) { Audacious::run_cmd($stuff) };
    Audacious::Cmds(1); #sync
    $Help = ""; #invalidate cache
}
...
sub export ($@) {
	...
	my($key,$val) = split /=/, $ex;
	if ($val) {
	    $ENV{$key} = ${"Audacious::$key"} = $val;
	}
	else {
	    *{"Audacious::$key"} = \$ENV{$key};
	}
    }
}

sub help { print {Audacious::pager()} Audacious::helpstr() }
...
sub quit {
    $remote->stop;
    Audacious::resume_config(1);
    $pconfig->write_file(Audacious::Config->perlfile);
    print Audacious::highlight(Msg => "Goodbye\n");
    $remote->quit;
    exit;
}

sub resume {
    my $str = Audacious::resume_config();
    return unless $str;
    my $resume = eval $str;

    if ($resume->{'files'}) {
	Audacious::playlist_load($resume->{'files'});
    }
    Audacious::sleep(0.25);
...
sub Audacious::resume_config {
    return if Audacious::playlist_is_empty(1);
    my $save = shift;
    if ($save) {
	my $resume = {
		      'files' => "$ENV{HOME}/.audacious/perl_resume_files",
	...
	Audacious::List::save($resume->{'files'}, 1);
    }
    else {
...
sub Audacious::History::clear { @history = () }
...
sub history ($;$) {
	...
	if ($args =~ /^\d+$/) {
	    ...
	    return Audacious::filecomplete($args, \&Audacious::historysel);
	}
	else {
    ...
    if (my $meth = Audacious::History->can($args)) {
	return $meth->();
    }
	...
	close $fh;
	Audacious::historysel(Audacious::dirname($args)) unless $args =~ /\.perlrc$/;
    }
    elsif ($args =~ s/^>//) {
	my $fh = Audacious::open_careful($args);
...
sub Audacious::WindowCmds { \@windows }
my %windows = map { $_,1 } @windows;

sub Audacious::Usage::window {
    my $opts = join '|', @windows;

    print Audacious::highlight(Error => 
...
my @BalanceCmds = sort grep { Audacious::Balance->can($_) } keys %Audacious::Balance::;
sub Audacious::BalanceCmds { \@BalanceCmds }

sub Audacious::Usage::balance {
...
sub balance ($;$$) {
    ...
    if ($subcmd) {
	if (Audacious::Balance->can($subcmd)) {
	    Audacious::Balance->$subcmd($val);
	}
	else {
	    Audacious::Usage->balance;
	}
    }
    else {
	my $bal = $remote->get_balancestr;
	print Audacious::highlight(Msg => "$bal\n");
    }
}
...
sub volume ($;$) {
	...
	Audacious::rl_termchar('');
	return Audacious::clearscreen($is_cpl) if $slide;
    }
    elsif ($args eq '+') {
	$remote->set_main_volume(++$vol);
	Audacious::rl_termchar('');
	...
	Audacious::waitfor(sub { $remote->get_main_volume != $vol });
    }

    $vol = $remote->get_main_volume;
    print Audacious::highlight(Msg => "    $vol%    \n");
}

sub next { 
    Audacious::change_pos(sub { $remote->playlist_next });
    current() if $_[0];
}

sub prev { 
    Audacious::change_pos(sub { $remote->playlist_prev });
    current() if $_[0];
}
...
sub change ($;$) {
    ...
    if ($args) {
	Audacious::init_songchange();
	$use_sc = ($args =~ /^on$/i);
	my $meth = $use_sc ? "run" : "stop";
	$sc->$meth();
    }
    my $on_off = $use_sc ? "is running" : "stopped";
    print Audacious::highlight(Msg => "SongChange thread $on_off\n");
}

sub current {
    return if Audacious::playlist_is_empty();

    my $track = $remote->get_playlist_pos;
    print Audacious::highlight(Msg => sprintf "%d - %s\n", $track+1, 
			  $remote->get_playlist_title($track)); 

    my($rate, $freq, $nch) = $remote->get_info;
    print Audacious::highlight(Msg => sprintf "[%s] [%d kbps][%d kHz][%s]\n", 
...
sub eject   { shift; Audacious::CD->new->eject(@_) }

sub Audacious::urlcomplete {
    my $arg = shift;
    if ($arg eq '-') {
	if (my $url = Audacious::urlsel()) {
	    Audacious::rl_termchar('');
	    return $url;
	}
    }
...
my $xmms_scalars = sub {
    package Audacious;
    no strict;
    grep { defined $$_ } keys %Audacious::;
};

sub Audacious::interp {
    my $string = shift;
    $Audacious::file = $remote->get_playlist_file;
    eval "{package Audacious; no strict; qq($string)}";
}

sub Audacious::filecomplete {
    my $arg = shift;

    if (my $earg = Audacious::interp($arg)) {
	$arg = $earg;
    }
    else {
	if ($arg =~ s/^\$//) {
    ...
    my $dpath = shift || \&Audacious::filesel_path;
    if ($arg eq '-') {
	if (my $path = $dpath->()) {
	    $path =~ s:/$::;
	    Audacious::rl_termchar('/');
	...
	if (my $path = Audacious::historysel()) {
	    $path =~ s:/$::;
	    Audacious::rl_termchar('/');
	    return $path;
	}
...
sub delete ($$) {
    ...
    for (sort { $b <=> $a } @{ Audacious::range($args) }) {
...
sub add ($$) {
    ...
    my(@cpl) = Audacious::filecomplete($args);
    return @cpl if $is_cpl;

    if ($args) {
	if (-d $args) {
	    Audacious::filesel_path($args);
	}
...
sub url ($$) {
    ...
    my(@urls) = Audacious::urlcomplete($args);
    return @urls if $is_cpl;

    if ($args) {
	Audacious::urlsel($urls[0]);
	for (@urls) {
...

{
	...
	while (<FH>) {
	    ...
	    my $fsize = -s _;
	    $size += $fsize;
	    printf "%s %s\n", Audacious::basename($_), Audacious::size_string($fsize);
	}
	close FH;
	printf "Total: %s\n", Audacious::size_string($size);
    }
	...
	my $fh = Audacious::open_careful($file);
	...
	if (close $fh) {
	    print Audacious::highlight(Msg => "playlist saved\n") unless $is_config;
	}
    }
}

my @ListCmds = sort grep { Audacious::List->can($_) } keys %Audacious::List::;
sub Audacious::ListCmds { \@ListCmds }
...
sub list ($$) {
    ...
    if ($subcmd && $subargs) {
	if (my $meth = Audacious::List->can($subcmd)) {
	    my(@files) = Audacious::filecomplete($subargs);
	    if ($is_cpl) {
		return @files;
	    }
	    $meth->(@files);
	...
	if ($args) {
	    my(@files) = Audacious::filecomplete($args);
	    if ($is_cpl) {
	    ...
	    Audacious::playlist_load(@files);
	}
    }
}

my @SortCmds = sort grep { Audacious::Sort->can($_) } keys %Audacious::Sort::;
sub Audacious::SortCmds { \@SortCmds }
...
sub sort ($$) {
    ...
    unless ($is_cpl) {
	if (my $meth = Audacious::Sort->can($args)) {
	    $meth->($subargs);
	}
	else {
	    print Audacious::highlight(Error => "unknown sort method: $args\n");
	}
	return;
    }
...
sub play ($;$) { 
	...
	if ($args) {
	    ...
	    $remote->playlist([Audacious::filecomplete($args)]);
	}
	else {
	    $remote->play;
	}
    ...
    Audacious::filecomplete($args);
}

my @DigCmds = ();
sub Audacious::DigCmds { 
    @DigCmds = (@{Audacious->InfoCmds}, 'file') unless @DigCmds;
    return \@DigCmds;
}
...
sub dig ($$;$) {
    ...
    if ($is_cpl) {
	if (my(@by) = grep /^$path/, @{Audacious::DigCmds()}) {
	    return @by;
	}
	elsif (my(@pm) = Audacious::filecomplete($path)) {
	    return @pm;
	}
	return;
    }
    ...
    my(@list, $search_by);
    my $dir = Audacious::filesel_path();

    if (-e $path) {
	$dir = $path;
	$args = $pat;
    }
    elsif (grep { $path eq $_ } @{Audacious::DigCmds()}) {
	$search_by = uc $path;
	$args = $pat;
    }   
    
    unless ($args) {
	print Audacious::highlight(Error => "no search pattern specified\n");
	return;
    }

    print Audacious::highlight(Msg => "searching in $dir...\n");
    my $depth = 0;
    Audacious::finddepth(sub {
			 my $file = "$File::Find::dir/$_";
			 if (++$depth >= $pause) {
			     print Audacious::highlight(Msg =>
...
sub track ($;$) {
    ...
    if ($args and $args =~ /\D+/) {
	@range = map { $_-1 } @{ Audacious::range($args) };
	$args = "";
    }
	...
	for (@range) {
		...
		push @retval, Audacious::highlight(Msg => "$desc [$time]$extra");
	    }
	    else {
...
sub titles ($$) {
    my($self, $args) = @_;
    return if $is_cpl;
    Audacious::playlist_do("title", $args);
}

sub files ($$) {
    my($self, $args) = @_;
    return if $is_cpl;
    Audacious::playlist_do("file", $args);
}
...
sub time ($;$) {
	...
	return ("0:00..$str");
    	...
    	print Audacious::highlight(Msg => $remote->get_output_timestr."\n");
    }
    elsif ($args =~ /^\d+$/) {
	...
	Audacious::sleep(0.10);
	my $ostr = $remote->get_output_timestr;
	print Audacious::highlight(Msg => "jumped to $ostr\n");
    }
    else {
	print Audacious::highlight(Error => "unknown time format: $args\n");
    }
}

{
    package Audacious::Info;
	...
	*$meth = sub {
		...
		$tag->{$key} = $val;
		if (-w $file) {
		    Audacious::set_mp3tag($file, $tag);
		    print Audacious::highlight(Msg => "$meth set to `$val'\n");
		}
		else {
		    print Audacious::highlight(Error => "$file is not writable\n");
		}
	    }
	    else {
		Audacious::info_print("\u$meth" => $tag->{$key});
	    }
	}
    }
}

my @InfoCmds = sort grep { Audacious::Info->can($_) } keys %Audacious::Info::;
sub Audacious::InfoCmds { \@InfoCmds }

sub Audacious::info_print {
    my($key, $val) = @_;
    $val ||= '?';
...
sub info ($$) {
	...
	unless ($want_subcmd) {
		...
		return Audacious::filecomplete($track);
	    }
	}
	...
	print Audacious::highlight(Error => "no such track\n");
	return;
    }
	...
	#print Audacious::highlight(Error => "choose track 1..$len ($pos)\n");
	return;
    }

    $file = $remote->get_playlist_file($track-1) unless $file;
    unless (-e $file) {
	print Audacious::highlight(Error => "$file does not exist: $!\n");
	return;
    }
    my $tag  = Audacious::get_mp3tag($file)  || {};
    my $info = Audacious::get_mp3info($file) || {};

    if ($subcmd) {
	if (Audacious::Info->can($subcmd)) {
	    Audacious::Info->$subcmd($tag, $file, $subargs);
	}
	else {
	    print Audacious::highlight(Error => 
    ...
    stat $file;
    Audacious::info_print(File => $file);
    Audacious::info_print(Size => Audacious::size_string(-s _));
    Audacious::info_print(Modified => scalar localtime((stat _)[9]));

    for (@InfoCmds) {
	Audacious::Info->$_($tag);
    }

    Audacious::info_print(Time => "$info->{MM}:$info->{SS}");
    for (qw(FREQUENCY STEREO BITRATE LAYER VERSION)) {
	Audacious::info_print("\u\L$_" => $info->{$_});
    }
}
...
Audacious - Interactive remote control shell for xmms

=head1 SYNOPSIS

  perl -MAudacious -e shell

=head1 DESCRIPTION

Audacious::shell provides an alternative or companion interface to the
audacious gui.
...
See also: I<Audacious::SongChange>.

If the file I<~/.audacious/.perlrc> exists, it will be automatically run
as a history script when the shell is invoked, before the prompt
loop.  For example, my I<~/.audacious/.perlrc> file looks like so:
...
is also saved to your ~/.audacious/config file, so it is always available.
Example:
...
This command will restore the audacious state to where it was just before
the last I<quit> command was run.  That is, it will load the saved
playlist and jump to the list position and output time where audacious was
just before quitting. 
The playlist, position and output time are saved in I<~/.audacious/config>.
...
If I<Audacious::shell> finds the I<$ENV{HOME}/.audacious/.perlurldb> file when
...
This command is used to I<show> or I<hide> the audacious windows.
If the shell is started and audacious is not already running, all windows
...
The following key bindings will toggle the audacious windows:
...
xmms(1), Audacious::SongChange(3), Audacious::Remote(3), Audacious::Config(3), MPEG::MP3Info(3)
